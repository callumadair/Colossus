...
public class RoleManagement extends BotAction {
  ...
  private void addRole(MessageCreateEvent event) {
    ...
    if (event.getMessageContent().toLowerCase().contains(getBot().getPrefix() + "addrole")
      ...
      for (User user : newUsers) {
        ...
        roleMessage.setDescription(
            user.getMentionTag() + " now has the following roles:\n" + roles.toString());
  ...
  /**
   * Sets the mentioned roles from the event to every new join in the discord server.
   *
   * @param event the specified event.
   */
  ...
  /**
   * Deletes all the roles beneath the application is able to from the event discord server.
   *
   * @param event the specified event.
   */
  ...
  /**
   * Creates a new administrator role in the specified event server with the specified role name and
   * colour.
   *
   * @param event the specified event.
   */
  private void createAdminRole(MessageCreateEvent event) {
    ...
    if ((event.getMessageAuthor().isServerAdmin())
        && (event
            .getMessageContent()
            .toLowerCase()
            .contains(getBot().getPrefix() + "createadminrole"))) {

      RoleBuilder newRole =
          Objects.requireNonNull(event.getServer().orElse(null)).createRoleBuilder();
  ...
  /**
   * Gives the roles mentioned "moderator" permissions in the specified event's discord server.
   *
   * @param event the specified event.
   */
  ...
  /**
   * Creates a muted role, an ordinary member role and updates the permissions of the everyone role
   * in the specified event's discord server.
   *
   * @param event the specified event.
   */
  private void createMutedRole(MessageCreateEvent event) {
    ...
    if (event.getMessageAuthor().isServerAdmin()
        && event
            .getMessageContent()
            .toLowerCase()
            .contains(getBot().getPrefix() + "createmutedrole")) {

      PermissionsBuilder mutedPerms = new PermissionsBuilder();
      mutedPerms.setDenied(
          PermissionType.SEND_MESSAGES, PermissionType.SPEAK, PermissionType.STREAM);
  ...
  private void muteUser(MessageCreateEvent event) {
    if (event.getMessageAuthor().isServerAdmin()
      ...
      for (User user : newMutes) {
        ...
        muteMessage
            .setColor(Color.RED)
            .setFooter("Muted by " + event.getMessageAuthor().getName())
    ...
    if (event.getMessageAuthor().isServerAdmin()
      ...
      for (User user : newUnmutes) {
        ...
        unmuteMessage
            .setColor(Color.BLUE)
            .setFooter("Unmuted by " + event.getMessageAuthor().getName())
