...
public class Invites extends BotAction {
  ...
  /**
   * Sends an invite to the event's discord server to all users mentioned in the message.
   *
   * @param event the specified event.
   */
  private void sendServerInvite(MessageCreateEvent event) {
    if (event
        .getMessageContent()
        .toLowerCase()
        .contains(getBot().getPrefix() + "sendserverinvite")) {
  ...
  /**
   * Creates an embed builder containing a server invite for the server the specified event occurred
   * in.
   *
   * @param event the specified event.
  ...
  private EmbedBuilder buildServerInvite(MessageCreateEvent event) {
    InviteBuilder inviteBuilder =
        new InviteBuilder(event.getChannel().asServerChannel().orElse(null));
  ...
  /**
   * Sends an invite for the bot user to all users mentioned in the specified event message.
   *
   * @param event the specified event.
   */
  ...
  private EmbedBuilder buildBotInvite(MessageCreateEvent event) {
    ...
    botInvite
        .setTitle("Invite Colossus here!")
        .setUrl(getBot().getApi().createBotInvite(colossusPerms))
        .setAuthor(event.getMessageAuthor())
        .setFooter("Made by Cal")
        .setTimestampToNow()
        .setColor(getBot().getRoleColour());
